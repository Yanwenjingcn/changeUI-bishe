 ...
 * <p>
 * 修改内容：
 * 1、在调度就绪队列时，选择 开始时间最早的 任务作为调度对象(如果有相同的开始时间，则是以随机的方式选取处理器)
 *    实验结果来看，其实与版本一，没有差别
 */
public class WorkflowBasedEdition2 {
    ...
    private static int chooseReadyListIndex(int[][] message, ArrayList<Task> readylist) {
        ...
        ArrayList<Integer> target_time_arr = new ArrayList<Integer>();
        for (int i = 0; i < readylist.size(); i++) {
            if (message[i][1] == earliestStart_Time) {
                target_time_arr.add(i);//加入其中的是任务在就绪队列中的编号
            }
        }


        int index_arr_Index = (int) (Math.random() * target_time_arr.size());
        target_Index = target_time_arr.get(index_arr_Index);
    ...
    public static boolean scheduleReadyTasks(DAG dagmap, ArrayList<Task> readylist) throws Exception {
        ...
        do {
            ...
            //需要调度的任务在就绪队列中的编号
            int target_readyListIndex = chooseReadyListIndex(message, readylist);
...
//        }
    ...
    public static void outputResult(Long diff, String resultPath) throws IOException {
        ...
        System.out.println("WorkFlowBasedEdition2:");
    ...
    public void runMakespan(String pathXML, String resultPath) throws Throwable {
        ...
        // 初始化作业映射
        WorkflowBasedEdition2 fb = new WorkflowBasedEdition2();
