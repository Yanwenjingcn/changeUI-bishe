 ...
 * <p>
 * 修改内容：
 * 1、在调度就绪队列时，选择 开始时间最早的 任务作为调度对象(如果有相同则不是随机选取而是选的最后一个)
 *
 *
 *
 *
 * 注意啊喂，dagdependmap里面的值是有问题的，如果一个任务有多个子任务，最后里面存的value只是最后一个子任务的id
 * DAGDependValueMap这个是没问题的。
 */
public class WorkflowBasedEdition1_bac {
    ...
    //初始化
    public WorkflowBasedEdition1_bac() {
...
//        }
       ...
       // Element adag = doc.getRootElement();
        ...
        for (int j = 0; j < tasknumber; j++) {
         ...
         //   System.out.println("path="+path);
            ...
            int tasklength = Integer.valueOf(attribute.getValue()).intValue();
            dag.setlength(tasklength);
            dag.setts(tasklength);
            dag_persional.setlength(tasklength);
            dag_persional.setts(tasklength);
            ...
            //这下面是考虑处理器上各自不同的计算能力的情况下（目前用不上）
            int sum = 0;
            int[] bufferedDouble = new int[PEList.size()];
            for (int k = 0; k < PEList.size(); k++) {
                bufferedDouble[k] = Integer.valueOf(tasklength / PEList.get(k).getability());
                sum = sum + Integer.valueOf(tasklength / PEList.get(k).getability());
            }
        ...
        for (int k = 0; k < list1.size(); k++) {
            Attribute output_target = (Attribute) list1.get(k);
            String[] pre_suc = output_target.getValue().split("_");
            ...
            XPath path2 = XPath.newInstance("//uses[@file='" + output_target.getValue() + "']/@size");
    ...
    public static void outputResult(Long diff, String resultPath) throws IOException {
        ...
        System.out.println("WorkFlowBasedEdition1_bac:");
    ...
    protected static void printInfile() throws IOException {
        String path = "D:\\edition1.txt";

        BufferedWriter out = null;
        try {
            out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(path, true)));
            out.write(rateResult[0][0] + "\t" + rateResult[0][2] + "\r\n");
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            try {
...
//                    System.out.println("dag.getdagid="+ message[dagcount][0]+"  dag.getid()="+message[dagcount][1]+"    PEId="+message[dagcount][2]
//                    +"  starttime="+message[dagcount][3]+"  endtime="+message[dagcount][4]);
                }
            }
        }
    }
    ...
    public void runMakespan(String pathXML, String resultPath) throws Throwable {
        ...
        // 初始化作业映射
        WorkflowBasedEdition1_bac fb = new WorkflowBasedEdition1_bac();
