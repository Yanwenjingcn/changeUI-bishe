 ...
 *
 * 注意啊喂，DAGDependValueMap这个是没问题的,里面的value值是在xml中随机生成的，就不是父任务的执行时长之类的了。
 */
public class WorkflowBasedEdition1 {
     ...
     */
    public static void scheduleDAG(int i, HashMap<Integer, ArrayList> SlotListInPestemp, HashMap<Integer, HashMap> TASKListInPestemp) throws Exception {
...
//        }
    ...
    private static int initDAG_createDAGdepend_XML(int i, int preexist, int tasknumber, int arrivetimes, String pathXML) throws NumberFormatException, IOException, JDOMException {
        ...
        for (int k = 0; k < list1.size(); k++) {
           ...
           // int[] presuc = new int[2];
            int preId = Integer.valueOf(pre_suc[0]).intValue() + preexist;
            int childId= Integer.valueOf(pre_suc[1]).intValue() + preexist;
            ...
            DAGDependValueMap.put((preId + " " + childId), (double) datasize);
            Task_queue.get(preId).addToSuc(childId);
            Task_queue.get(childId).addToPre(preId);
    ...
    public static void outputResult(Long diff, String resultPath) throws IOException {
        ...
        System.out.println("WorkFlowBasedEdition1:");
...
//                    +"  starttime="+message[dagcount][3]+"  endtime="+message[dagcount][4]);
    ...
    public void runMakespan(String pathXML, String resultPath) throws Throwable {
        ...
        // 初始化作业映射
        WorkflowBasedEdition1 fb = new WorkflowBasedEdition1();
        ...
        for (int i = 0; i < DAGMapList.size(); i++) {
            ...
            //开始其它作业调度
            scheduleDAG(i, SlotListInPestemp, TASKListInPestemp);
        }
